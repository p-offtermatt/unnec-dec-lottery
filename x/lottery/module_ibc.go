...
func (im IBCModule) OnRecvPacket(
	...
	var modulePacketData types.LotteryPacketData
	...
	case *types.LotteryPacketData_RefundLotteryPacket:
		packetAck, err := im.keeper.OnRecvRefundLotteryPacket(ctx, modulePacket, *packet.RefundLotteryPacket)
		if err != nil {
		...
		ctx.EventManager().EmitEvent(
			sdk.NewEvent(
				types.EventTypeRefundLotteryPacket,
				sdk.NewAttribute(sdk.AttributeKeyModule, types.ModuleName),
				sdk.NewAttribute(types.AttributeKeyAckSuccess, fmt.Sprintf("%t", err != nil)),
			),
		)
	case *types.LotteryPacketData_SayhelloPacket:
		packetAck, err := im.keeper.OnRecvSayhelloPacket(ctx, modulePacket, *packet.SayhelloPacket)
		if err != nil {
			ack = channeltypes.NewErrorAcknowledgement(err)
		} else {
			// Encode packet acknowledgment
...
func (im IBCModule) OnAcknowledgementPacket(
	...
	switch packet := modulePacketData.Packet.(type) {
	case *types.LotteryPacketData_RefundLotteryPacket:
		err := im.keeper.OnAcknowledgementRefundLotteryPacket(ctx, modulePacket, *packet.RefundLotteryPacket, ack)
		if err != nil {
			return err
		}
		eventType = types.EventTypeRefundLotteryPacket
	case *types.LotteryPacketData_SayhelloPacket:
		err := im.keeper.OnAcknowledgementSayhelloPacket(ctx, modulePacket, *packet.SayhelloPacket, ack)
		if err != nil {
			return err
		}
...
func (im IBCModule) OnTimeoutPacket(
	...
	switch packet := modulePacketData.Packet.(type) {
	case *types.LotteryPacketData_RefundLotteryPacket:
		err := im.keeper.OnTimeoutRefundLotteryPacket(ctx, modulePacket, *packet.RefundLotteryPacket)
		if err != nil {
			return err
		}
	case *types.LotteryPacketData_SayhelloPacket:
